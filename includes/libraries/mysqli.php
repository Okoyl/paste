...
*/
require_once("/home/websites/Domains/squazza/dev/kint/Kint.class.php");
...
class DB
{
    ...
    {
        global $CONF;
        $this->dblink = ($GLOBALS["___mysqli_ston"] = mysqli_connect(
            $CONF["dbhost"],
            $CONF["dbuser"],
            $CONF["dbpass"]))
        or die("Unable to connect to the database. Have you installed PASTE?");

        ((bool)mysqli_query($this->dblink, "USE " . $CONF["dbname"]))
    ...
    function _query($sql)
    {
        ...
        if (func_num_args() > 1) {
            ...
            for ($i = 1; $i <= count($args); $i++) {
                if (isset($args[$i])) {
                    $sql = preg_replace("/$q/", "'" . preg_quote(((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $args[$i]) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""))) . "'", $sql, 1);
                }
        ...
        if (!$this->dbresult) {
            die("Query failure: " . ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)) . "<br />$sql");
        }
